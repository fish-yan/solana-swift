...
class APIClientTests: XCTestCase {
    ...
    func testGetBalance() async throws {
        ...
        let apiClient = JSONRPCAPIClient(endpoint: endpoint, networkManager: mock)
        let result: UInt64 = try! await apiClient.getBalance(account: "", commitment: "finalized")
...
//        XCTAssert(result[0].pubkey == "57UtuDwoCurTTWySMeV5MiopvDWvK2QeLWu47biQjjLJ")
    ...
    func getFeeForMessage() async throws {
        let mock = NetworkManagerMock(NetworkManagerMockJSON["getFeeForMessage"]!)
        let apiClient = JSONRPCAPIClient(endpoint: endpoint, networkManager: mock)
        let result = try! await apiClient.getFeeForMessage(message: "7jvToPQ4ASj3xohjM117tMqmtppQDaWVADZyaLFnytFr", commitment: nil)
        XCTAssertNotNil(result)
